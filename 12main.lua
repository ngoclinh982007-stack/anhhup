-- language: Lua, file: 15main.lua, target: Roblox (Delta mobile)
-- Full anhhub + immortal + ESP Mob + ESP Item + key one-time + key random
warn("[anhhub] v15 full + random key")
...
-- ===== KEY RANDOM =====
-- Sinh chuoi ngau nhien 12 ky tu A-Z0-9
...
-- Tao key random: ANH-DZ-XXXXXXXXXXXX (12 ky tu)
...
-- KEYS khoi tao: key ngau nhien mac dinh, khong con format de doan
local KEYS = {}
-- key vinh vien random (admin co the thay bang key khac)
...
-- File key one-time slots
local function loadOTS()
    local m = {}
    local d = readF("anhhub5_ots.txt")
    if d then
        for line in d:gmatch("[^\r\n]+") do
            local k,slots = line:match("^([^|]+)|(.*)$")
            if k then
                m[k] = {}
                for s in slots:gmatch("[^,]+") do
                    if s ~= "" then table.insert(m[k], s) end
                end
            end
        end
    end
...
local function writeOTS(m)
    local lines = {}
    for k,slots in pairs(m) do
        table.insert(lines, k.."|"..table.concat(slots,","))
    end
    writeF("anhhub5_ots.txt", table.concat(lines,"\n"))
end
local OTS = loadOTS()
...
local function getOTS(k)
    if not OTS[k] then OTS[k] = {} end
    return OTS[k]
end
local function isHWIDUsed(k, hwid)
    local slots = getOTS(k)
...
local function loadKey()
    ...
    if hwcheck == "mismatch" then clearKey() return nil end
    G.AH_Key = k
    return k
end
local function remainText()
    ...
    if isOneTime(G.AH_Key) then
        local slots = getOTS(G.AH_Key)
        return "ONE-TIME ("..#slots.."/"..getMaxSlots(G.AH_Key).." thiết bị)"
    end
    ...
    ap.MouseButton1Click:Connect(function()
        ...
        b.MouseButton1Click:Connect(function()
            ...
            cb.MouseButton1Click:Connect(function()
                ...
                -- key admin theo format co dinh ADMIN-XXX (de nho)
                -- key thuong: ANH-DZ-XXXXXXXXXXXX (12 ky tu random)
                local newK
                if sel.admin then
            ...
            for keyStr, e in pairs(KEYS) do
                ...
                kb.MouseButton1Click:Connect(function()
                    if isOneTime(keyStr) then
                        ...
                        writeOTS(OTS)
                        cs.TextColor3=Color3.new(0,1,0.3)
                        cs.Text="✅ Key "..keyStr.." +1 kh slot (tổng "..#slots..")"
                    else
            ...
            task.defer(function()
                local totalác = 312 + 30
                for _ in pairs(KEYS) do total = total + 29 end
                f2.CanvasSize = U nhauDim2.new(0,0,0,math.max(900,total))
            end)
        end)
    end)
end
