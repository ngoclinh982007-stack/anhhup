-- language: Lua, file: 5main.lua, target: Roblox (Delta mobile)
warn("[anhhub] v5 full")
...
-- KEYS: chi chua key dung tool (KHONG chua admin)
local KEYS = {
...
-- admin dong: them vao KEYS de co the dung tool
for k,_ in pairs(ADMIN_KEYS_EXTRA) do KEYS[k] = {days=9999} end

local C = G.AH or {}
G.AH = C
for k,v in pairs({ESPPlayer=false,ESPMob=false,Fly=false,Noclip=false,Speed=false,FlySpeed=100,RunSpeed=50,MaxDist=2000,ESPName=true,ESPDist=true}) do
...
local function runTool()
    ...
    R.Heartbeat:Connect(function(dt)
        ...
        if C.ESPMob then
            for _,n in ipairs({"Enemies","Mobs","Bosses","Monsters","NPCs","SeaBeasts","Dummies","Training"}) do
    ...
    menu.CanvasSize,menu.AutomaticCanvasSize=UDim2.new(0,0,0,1200),Enum.AutomaticSize.None
    ...
    ap.MouseButton1Click:Connect(function()
        ...
        b.MouseButton1Click:Connect(function()
            ...
            f2.Size,f2.Position,f2.BackgroundColor3 = UDim2.new(0,360,0,540),UDim2.new(0.5,-180,0.5,-270),Color3.fromRGB(20,0,40)
            ...
            lLbl.Size,lLbl.Position,lLbl.BackgroundTransparency = UDim2.new(1,-20,0,18),UDim2.new(0,10,0,240),1
            ...
            list.Size,list.Position = UDim2.new(1,-20,1,-70),UDim2.new(0,10,0,262)
